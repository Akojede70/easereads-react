...
export const passwordReset = async (payload: {email: string, password: string, otp: string}) => {
    const response = await axiosInstance.put('/api/v2/student/reset-password', payload);
    return response.data; 
};
...
export const ForgetPassword = async (email: string) => {
    const response = await axiosInstance.get(`/api/v2/student/forgot-password/${email}`,);
...
export const verification = async (payload: {email: string, otp: string}) => {
    const response = await axiosInstance.put('/api/v2/student/verify-email', payload);
    return response.data; 
};

export const verifyPassword = async (payload: {email: string, otp: string}) => {
    const response = await axiosInstance.post('/api/v2/student/reset-password-otp', payload);
    return response.data; 
};

export const resendEmail = async (email: string, ) => {
    const response = await axiosInstance.get(`/api/v2/student/resend-email/${email}`);
    return response.data; 
};
